...
def setup_logger(args):
    ...
    log_path = './logs/{0}_{1}_{2}.log'.format(args.model, args.sparsity, hashlib.md5(str(args_copy).encode('utf-8')).hexdigest()[:8])
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='./output/LTH/', help='fix topology')
    ...
    for i in range(args.iters):
        ...
        if args.sparse:
            ...
            print('load pre-trained ckpts')
            model.load_state_dict(torch.load(os.path.join(args.output_dir, "Initialization_seed{}".format(args.seed),
                                                          "pretrained_model_iter0.ckpt")))
